...
class LDAClustering():
    ...
    def pre_process_corpus(self, content, stopwords_path):
        """
        ...
        corpus = [self.cut_words(line.strip()) for line in content]
    ...
    def fmt_lda_result(self, lda_result):
        ...
        semi["class_label"] = label_list
        return ret

    def lda(self, content, n_components=5, learning_method='batch',
        ...
        :return:
        """
        cntTf = self.pre_process_corpus(content=content, stopwords_path=stopwords_path)
...
if __name__ == '__main__':
    ...
    semi = pd.read_csv("../data/unlabeled_data.csv")
    #semi = pd.read_csv("../data/test_data.csv")

    LDA = LDAClustering()
    ret = LDA.lda(semi["content"], stopwords_path='../data/stop_words.txt', max_iter=100, n_components=10)

    semi.to_csv("semi.csv", index=False)
    print(ret)